...
export function ContactEditModal({ open, contact, onCancel }: Props) {
	...
	const handleSubmit = async (values: UpdateContactRequest) => {
		if (!contact) {
			return;
		}
		try {
			await updateContact.mutateAsync({ id: contact.id, data: values });
			onCancel();
		} catch {
			// Handled by hook
		}
